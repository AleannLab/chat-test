...
import {
  ...
  isFax,
  isVoicemail,
} from 'components/Logs/activityType';
...
export class ActivityLog extends SingleSelectableResource {
  ...
  @observable initialSelection = true;
  ...
  //  for existing datum ID based implementation
  onSelect = action(() => {
    const selected = this.datum[this.selected];
    const activityType = getActivityType(Number(selected.log_type_id));

    if (!selected || selected.seen || activityType.itemLabel === 'Voicemail')
      return;

    const endpoint = getEndpoint(selected.log_type_id);
    if (endpoint === '/activity') return;
  ...
  //Mark All activity Logs as seen
  ...
  getQueryKeysByLogType = (logType) => {
    if (isCall(logType)) {
      return this.queryKeys.callLogs;
    }
    if (isVoicemail(logType)) {
      return this.queryKeys.voicemailLogs;
    }
    if (isFax(logType)) {
      return this.queryKeys.faxLogs;
    }
  };
