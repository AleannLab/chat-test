...
const BrandLogo = (props) => {
  ...
  const onCropComplete = async (blob) => {
    const file = new File([blob], `${officeProfile.data.office_name}_logo`, {
  ...
  const handleSave = async (file) => {
    await officeProfile.deleteOfficeImage();
    await officeProfile.fetchData();
    try {
      const response = await officeProfile.officeImageUpload(file);
      if (response.url) {
        axios({
          method: 'PUT',
          url: response.url,
          ...
          .then(async () => {
            await officeProfile.fetchData();
            notification.showSuccess('Company logo was updated successfully');
            setTimeout(() => {
          ...
          .catch((e) => {
            notification.showError(
              e?.DetailedMessage ||
                'An unexpected error occurred while attempting to fetch the company logo',
            );
          });
      }
    } catch (e) {
      notification.showError(
        e?.DetailedMessage ||
          'An unexpected error occurred while attempting to upload the company logo',
      );
    }
  };
    ...
    <div className={`${props.className} ${styles.officeImageRootContainer}`}>
      ...
      >
        {officeProfile.loaded ? (
          officeProfile.data.office_profile_pic && !uploading ? (
            ...
            <div className={styles.uploadContainer}>
              <Loader show={uploading} message="Uploading brand logo">
      ...
      {!!officeProfile.data.office_profile_pic && (
