...
export default function EditReminderSMSTemplateModal({ onClose }) {
  const { reminders, notification } = useStores();
  const queryClient = useQueryClient();

  // Get cached data from react-query
  const {
    kasper_reminder_general_sms_individual: smsTemplateForIndividual = '',
    kasper_reminder_general_sms_family: smsTemplateForFamily = '',
  } = queryClient.getQueryData(['officeConfigs', 'generalReminders']);
  ...
  const handleSubmit = async ({ message }, type) => {
    if (type === 'individual') {
      await updateOfficeConfigs.mutateAsync({
        kasper_reminder_general_sms_individual: message,
      });
    } else {
      await updateOfficeConfigs.mutateAsync({
        kasper_reminder_general_sms_family: message,
      });
    }
  };
