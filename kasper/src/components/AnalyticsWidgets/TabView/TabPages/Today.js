...
import { convertToCurrency, sanitizeNumber } from 'helpers/misc';
...
const Today = () => {
  ...
  // React query to fetch morning huddle data for today
  const {
    isLoading,
    isSuccess,
    data: morningHuddleData,
  } = useQuery(
    ['analytics', MORNING_HUDDLE_TYPE.TODAY],
    () => analyticsStore.getMorningHuddleData(MORNING_HUDDLE_TYPE.TODAY),
    {
      staleTime: 300000, // 5 minutes
    ...
    <Suspense fallback={<Skeleton />}>
      ...
      {isSuccess && (
        <>
          <div className="mb-3">
            <div>
              <span className={styles.pageTitle}>Today’s Numbers</span>
              &nbsp;&nbsp;
              <span className={styles.dateField}>
                {moment(morningHuddleData.date).format('dddd, MMMM DD, Y')}
              </span>
            </div>
            <div className={styles.pageSubTitle}>What’s going on today.</div>
          </div>

          <Grid container className={`${styles.shortDetails} mb-3`}>
            <Grid item xs={12} md={10}>
              We have {morningHuddleData.totalPatients} patients coming in.
              {productionGoalQuery.isSuccess &&
                !!productionGoalQuery.data &&
                morningHuddleData.totalProduction !== null && (
                  <span>
                    {' '}
                    Scheduled production is{' '}
                    {convertToCurrency(morningHuddleData.totalProduction)} which
                    is{' '}
                    {convertToCurrency(
                      Math.abs(
                        productionGoalQuery.data -
                          morningHuddleData.totalProduction,
                      ),
                    )}{' '}
                    {productionGoalQuery.data <
                    morningHuddleData.totalProduction
