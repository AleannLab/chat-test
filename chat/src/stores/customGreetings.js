...
export class CustomGreetings extends Resource {
  ...
  addGreeting = flow(function* (fileName, uuid, id) {
    try {
  ...
  updateGreetingName = flow(function* ({ id, name }) {
    try {
      ...
      throw 'An unexpected error occurred while attempting to update the greeting name';
    }
  });

  getGreetings = flow(function* (id) {
    try {
      const response = yield this.fetch(
        `${CONSTANTS.CALL_CONTROL_API_URL}/greeting-types/${id}`,
        {
          method: 'GET',
        },
      ).then((res) => res.json());
      if (response.success) {
        ...
        throw Error('An unexpected error occurred while fetching number list');
      }
    } catch (e) {
      throw 'An unexpected error occurred while fetching number list';
    }
  });

  getCustomGreetings = flow(function* () {
    try {
      const response = yield this.fetch(
  ...
  getOfficeNumbers = flow(function* () {
    try {
      const response = yield this.fetch(
        `${CONSTANTS.CALL_CONTROL_API_URL}/greeting-number`,
        {
          method: 'GET',
        },
      ).then((res) => res.json());
  ...
  postOfficeNumbers = flow(function* ({ method, ...rest }) {
    try {
      const response = yield this.fetch(
        `${CONSTANTS.CALL_CONTROL_API_URL}/greeting-number`,
        {
